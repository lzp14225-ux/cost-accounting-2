"""
WeightPriceCalculationHandler - 按重量计算模架价格处理器
负责人：人员B2
...
class WeightPriceCalculationHandler(BaseActionHandler):
    ...
    async def handle(
        ...
        Returns:
            ActionResult: 处理结果
        """
        logger.info(f"⚖️  处理按重量计算: {intent_result.raw_message}")
        ...
        try:
            ...
            if keyword:
                ...
                logger.info(f"未指定子图，将按重量计算所有 {len(subgraph_ids)} 个子图")
            else:
                # 将短名称转换为完整 ID
                subgraph_ids = self._resolve_subgraph_ids(subgraph_ids, context)
                logger.info(f"将按重量计算指定的 {len(subgraph_ids)} 个子图: {subgraph_ids}")
            ...
            await self._save_pending_action(job_id, {
                ...
                "api_url": settings.WEIGHT_PRICE_API_URL,
            ...
            if len(display_names) <= 5:
                message = f"将按重量计算以下子图的模架价格：{', '.join(display_names)}，请确认"
            else:
                message = f"将按重量计算 {len(display_names)} 个子图的模架价格（{', '.join(display_names[:3])} ...），请确认"
            
            logger.info(f"✅ 按重量计算请求准备完成")
            ...
            logger.error(f"❌ 处理按重量计算失败: {e}", exc_info=True)
            return ActionResult(
                status="error",
                message=f"处理按重量计算请求失败：{str(e)}",
                data={}
            )
