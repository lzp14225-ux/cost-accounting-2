...
class PriceCalculationHandler(BaseActionHandler):
    """
    价格计算处理器
    ...
    def __init__(self):
        """初始化 Handler"""
        super().__init__()
        logger.info("✅ PriceCalculationHandler 初始化完成")
    ...
    async def handle(
        ...
        Returns:
            ActionResult: 处理结果
        """
        logger.info(f"💰 处理价格计算: {intent_result.raw_message}")
        ...
        try:
            ...
            if keyword:
                ...
                logger.info(f"未指定子图，将计算所有 {len(subgraph_ids)} 个子图")
            else:
                # 🆕 将短名称转换为完整 ID
                subgraph_ids = self._resolve_subgraph_ids(subgraph_ids, context)
                logger.info(f"将计算指定的 {len(subgraph_ids)} 个子图: {subgraph_ids}")
            ...
            if len(display_names) <= 5:
                message = f"将重新计算以下子图的价格：{', '.join(display_names)}，请确认"
            else:
                message = f"将重新计算 {len(display_names)} 个子图的价格（{', '.join(display_names[:3])} ...），请确认"
            
            logger.info(f"✅ 价格计算请求准备完成")
            ...
            logger.error(f"❌ 处理价格计算失败: {e}", exc_info=True)
            return ActionResult(
                status="error",
                message=f"处理价格计算请求失败：{str(e)}",
                data={}
            )
