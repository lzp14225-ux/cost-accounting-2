...
async def search_by_job_id(job_id: str, subgraph_ids: List[str] = None) -> Dict[str, Any]:
    ...
    Args:
        job_id: 任务ID (UUID字符串)
        subgraph_ids: 此参数被忽略（为保持接口一致性，水磨价格是全局配置）
...
async def _fetch_price_data(job_id: str) -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Fetch water_mill price data failed: {e}")
        raise
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python water_mill_search.py <job_id> [subgraph_ids...]")
        print("Note: subgraph_ids are ignored for water_mill search")
        sys.exit(1)
    ...
    print("\n--- L_Water Mill 价格列表 ---")
    for p in results["l_water_mill_prices"]:
        print(f"  sub_category: {p['sub_category']}, price: {p['price']}, unit: {p['unit']}, min_num: {p.get('min_num', 'N/A')}")
