...
async def calculate(
    ...
    logger.info(f"Calculating add auto material cost for job_id: {job_id}, parts count: {len(base_data.get('parts', []))}")
        ...
        updates_for_batch = [
            {
                ...
                "value": d["material_additional_cost"],
...
async def _calculate_part_cost(
    ...
    has_auto_material = part.get("has_auto_material")  # True/False
    ...
    # 判断是否是自找料（has_auto_material = True 表示自找料）
    if not has_auto_material:
        logger.info(f"Part {part_name} is not self-sourced material (has_auto_material=False), skipping")
        # 不是自找料，返回结果和数据库更新数据
        result = {
...
async def _update_calculation_details(
    job_id: str,
    subgraph_id: str,
    material_additional_cost: float,
    new_steps: List[Dict]
):
    """
    ...
    await batch_upsert_with_steps(
        [{
            "job_id": job_id,
            "subgraph_id": subgraph_id,
            "value": material_additional_cost,
            "steps": new_steps
        }],
        "add_auto_material",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python price_add_auto_material.py <job_id> <subgraph_id1> [subgraph_id2 ...]")
        sys.exit(1)
    ...
    for result in results["results"]:
        if "error" in result:
        ...
        elif result.get("has_auto_material"):
            print(f"\n零件: {result['part_name']} ({result['subgraph_id']})")
            print(f"  是自找料")
            ...
            print(f"  自找料材料费: {result['material_additional_cost']} 元")
        else:
            print(f"\n零件: {result['part_name']} ({result['subgraph_id']})")
            print(f"  不是自找料，无需计算额外材料费")
