...
3. 判断是大水磨还是小磨床
4. 计算油槽耗时费：oil_tank * price
5. 更新 processing_cost_calculation_details 表的 oil_tank_cost
"""
...
MCP_TOOL_META = {
    ...
    "description": "计算水磨油槽耗时费：根据零件water_mill数据和价格配置计算油槽费用",
    "inputSchema": {
        "type": "object",
        "properties": {
            "search_data": {
...
async def calculate(
    ...
    logger.info(f"Calculating water mill oil tank cost for job_id: {job_id}, parts count: {len(base_data.get('parts', []))}")
        ...
        updates_for_batch = [
            {
                ...
                "value": d["oil_tank_cost"],
                "steps": d["calculation_steps"]
            }
            for d in db_updates
        ]
        await batch_upsert_with_steps(updates_for_batch, "water_mill_oil_tank", "oil_tank_cost")
    ...
    for price in water_mill_data.get("s_water_mill_prices", []):
        ...
        if sub_category == "oil_tank":
            try:
                ...
                logger.warning(f"Failed to parse oil_tank price: {price_value}, error: {e}")
...
async def _calculate_part_price(
    ...
    logger.info(f"Calculating oil tank cost for part: {part_name} ({subgraph_id})")
    ...
    for detail in water_mill_details:
        if "oil_tank" in detail:
            oil_tank_count = detail.get("oil_tank", 0)
            break
    
    if oil_tank_count == 0:
        logger.info(f"No oil_tank for {part_name}")
        calculation_steps.append({
            "step": "检查oil_tank数据",
    ...
    oil_tank_unit_price = oil_tank_info.get("price", 0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python price_water_mill_oil_tank.py <job_id> <subgraph_id1> [subgraph_id2 ...]")
        sys.exit(1)
